...
class OCRProcessor {
     ...
     */
    static async extractTextFromImage(buffer, options = { includeMetadata: true, ocrConfidence: 0.6 }) {
        try {
            // Preprocess image for better OCR results
            const processedBuffer = await this.preprocessImage(buffer);
            // Perform OCR
            const { data: { text, confidence } } = await tesseract_js_1.default.recognize(processedBuffer, options.language || 'eng', {
                logger: m => {
     ...
     */
    static async extractTextFromImages(buffers, options = { includeMetadata: true, ocrConfidence: 0.6 }) {
        const results = [];
