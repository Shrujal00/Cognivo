...
const ocr_processor_1 = require("../utils/ocr-processor");
const text_processor_1 = require("../utils/text-processor");
class AIService {
    constructor() {
        this.rateLimitMap = new Map();
        (0, ai_config_1.validateConfig)();
        this.openai = new openai_1.default({
            apiKey: ai_config_1.aiConfig.openai.apiKey,
        });
    }
    /**
    ...
    async generateNotes(content, options) {
        try {
            ...
            const response = await this.openai.chat.completions.create({
                model: ai_config_1.aiConfig.openai.model,
                messages: [
                    {
                        role: 'system',
                ...
                max_tokens: ai_config_1.aiConfig.openai.maxTokens,
                temperature: ai_config_1.aiConfig.openai.temperature
            });
     ...
     */
    async answerQuestion(question, context, options = { context: '', language: 'en', includeSource: true }) {
        try {
    ...
    async transcribeAudio(audioBuffer, options) {
        try {
            ...
            return {
                ...
                timestamps: options.includeTimestamps ? response.words?.map((w) => ({
                    start: w.start,
                    end: w.end,
                    text: w.word
                })) : undefined,
     ...
     */
    async generateStudyRoadmap(options) {
        try {
     ...
     */
    async generatePerfectQuestions(content, options) {
        try {
     ...
     */
    async generateStudyImages(content, options) {
        try {
     ...
     */
    async generateResearchPDF(content, options) {
        try {
     ...
     */
    async extractTextFromPDF(buffer, options = { includeMetadata: true }) {
        try {
            return await pdf_parser_1.PDFParser.extractText(buffer, options);
        }
        catch (error) {
     ...
     */
    async extractTextFromImage(buffer, options = { includeMetadata: true, ocrConfidence: 0.6 }) {
        try {
            return await ocr_processor_1.OCRProcessor.extractTextFromImage(buffer, options);
        }
        catch (error) {
     ...
     */
    async extractTextFromDocument(buffer, mimeType, options = { includeMetadata: true }) {
        try {
            return await text_processor_1.TextProcessor.extractTextFromDocument(buffer, mimeType, options);
        }
        catch (error) {
    ...
    // Helper Methods
    buildNoteGenerationPrompt(content, options) {
...
Note: correctAnswer should be a string (A, B, C, or D) representing the correct option letter.
    ...
    buildQAPrompt(question, context, options) {
        return `
...
Answer:
    ...
    buildFlashcardGenerationPrompt(content, options) {
...
Format the response as JSON with the following structure:
{
  ...
  "tags": ["main-tag1", "main-tag2"]
    ...
    buildTranslationPrompt(content, targetLanguage, options) {
        return `
...
Translation:
    ...
    parseGeneratedNote(content, options) {
        try {
    ...
    parseGeneratedQuiz(content, options) {
        try {
    ...
    parseGeneratedFlashcards(content, options) {
        try {
            const parsed = JSON.parse(content);
            const now = new Date();
            return {
    ...
    calculateConfidence(answer, context) {
    ...
    generateFollowUpQuestions(originalQuestion, answer) {
    ...
    extractEducationalTerms(original, translated) {
    ...
    checkRateLimit(operation) {
        const now = Date.now();
        const key = `${operation}_${Math.floor(now / ai_config_1.aiConfig.rateLimit.windowMs)}`;
        const current = this.rateLimitMap.get(key) || { count: 0, resetTime: now + ai_config_1.aiConfig.rateLimit.windowMs };
        if (current.count >= ai_config_1.aiConfig.rateLimit.maxRequests) {
...
Format as JSON with this structure:
{
  ...
  "weeklySchedule": [
    ...
    buildPerfectQuestionPrompt(content, options) {
...
[
  {
    ...
    buildImageGenerationPrompt(content, options) {
        const styleInstructions = {
...
Focus on making the content visually accessible and educationally effective.
    ...
    buildResearchPDFPrompt(content, options) {
...
Format as JSON with this structure:
{
  ...
  "tableOfContents": [
    ...
    parseGeneratedRoadmap(content, options) {
            ...
            return {
                ...
                milestones: (parsed.milestones || []).map((m, index) => {
                    ...
                    if (m.resources && Array.isArray(m.resources)) {
                        resourceStrings = m.resources.map((r) => {
                            if (typeof r === 'string')
                                return r;
                            if (r.title && r.type) {
                    ...
                    if (m.tasks && Array.isArray(m.tasks)) {
                        formattedTasks = m.tasks.map((task, taskIndex) => {
    ...
    parseGeneratedQuestions(content, options) {
        try {
            const parsed = JSON.parse(content);
            const now = new Date();
    ...
    parseGeneratedResearchPDF(content, options) {
        try {
            const parsed = JSON.parse(content);
            const now = new Date();
            return {
