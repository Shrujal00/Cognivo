...
import { swaggerMiddleware, swaggerUiMiddleware } from './middleware/swagger';
...
app.post('/api/ai/generate-notes', async (req, res) => {
  try {
    const { content, options }: { content: string; options: NoteGenerationOptions } = req.body;
...
app.post('/api/ai/generate-quiz', async (req, res) => {
  try {
    const { content, options }: { content: string; options: QuizGenerationOptions } = req.body;
...
app.post('/api/ai/transcribe-audio', upload.single('audio'), async (req, res) => {
  try {
    ...
    const options: AudioToTextOptions = req.body.options ? JSON.parse(req.body.options) : {
...
app.post('/api/ai/generate-roadmap', async (req, res) => {
  try {
    const { options }: { options: RoadmapGenerationOptions } = req.body;
